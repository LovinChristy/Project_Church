...
<a href="<?php echo base_url()."index.php/AdminController/viewApprovedRequest"?>">View Approved Requests</a> ||
    ...
    <table class="table table-responsive  table-hover" cellpadding="0" cellspacing="0" border="0" >
     ...
     if($donation){
         ...
         ?>
     <tr><td><?=$ch->donation_id?></td><td><?=$ch->name;?></td><td><?=$ch->msg;?></td><td><?=$ch->accountno;?></td><td><?=$ch->contact;?></td><td><?=$ch->email;?></td>
</td><td><a href="<?php echo base_url()."index.php/AdminController/rejectrequest/".$ch->donation_id ?>"class="btn btn-danger">Remove</a>
</td>
